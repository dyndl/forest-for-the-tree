...
export async function generateMorningBriefWithOura({ tasks, calendarEvents, emails, roadmap, ouraData, userContext, currentHour, localDate, localTomorrow, vetoHistory = '', agentContributions = '', overdueTasks = [] }) {
  ...
  const emailSummary = sortedBundleKeys.length > 0
    ? sortedBundleKeys.map(cat => {
        ...
        const items = bundle.slice(0, 3).map(e => `  • "${e.subject}" — from ${e.from}`).join('\n')
...
Respond ONLY with this JSON (no markdown, no preamble):
{
  ...
  const raw = await callClaude(prompt, buildCooSystem(userCtx?.adhd_aware ?? userContext?.adhd_aware ?? false), 3200)
  const parsed = parseJSON(raw)
