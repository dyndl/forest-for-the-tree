...
export async function generateMorningBriefWithOura({ tasks, calendarEvents, emails, roadmap, ouraData, userContext, currentHour, localDate, localTomorrow, vetoHistory = '', agentContributions = '', overdueTasks = [], stream = false, llmKeys = {} }) {
  ...
  // scheduleWindow end is computed after userCtx (see below) — daily_blocks may pull it earlier
...
${allBlocksForDay.map(b => {
  ...
  const scheduleWindow = `${fmt(startHour)} to ${strictEveningBlock ? fmtBlock(strictEveningBlock.start) : '8:00 PM'}`
...
- DAILY BLOCKS — STRICT: NEVER schedule any task during a [STRICT] block. These windows are immovable (sleep, fixed commitments). The schedule must begin only after any strict block whose end falls in the morning.
