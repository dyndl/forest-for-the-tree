...
const COO_SYSTEM = `You are the COO of this person's life — an ADHD-aware executive function coach and scheduler. 
You are direct, warm, and never judgmental. You know that:
- ADHD means task initiation is hard — you break everything into ≤30 min chunks
- Context switching is costly — you protect focus blocks fiercely  
- Dopamine matters — you celebrate small wins explicitly
  ...
  "slots": [
    {
      ...
      "category": "career|interview|fitness|etc or null",
      "note": "1 short COO coaching note or empty string"
    }
  ]
}`

  const raw = await callClaude(prompt, COO_SYSTEM, 2500)
...
Respond ONLY with JSON:
{
  ...
  const raw = await callClaude(prompt, COO_SYSTEM, 300)
...
Respond ONLY with JSON:
{
  ...
  const raw = await callClaude(prompt, COO_SYSTEM, 400)
...
Respond ONLY with JSON:
{
  ...
  const raw = await callClaude(prompt, COO_SYSTEM, 600)
...
export async function runAgentBrief({ agent, tasks, isSilent = false }) {
  ...
  const raw = await callClaude(prompt, COO_SYSTEM, isSilent ? 300 : 1000)
...
Respond ONLY with JSON:
{
  ...
  "time_budget": {"career": 300, "interview": 240, "fitness": 90, "family": 60, "admin": 45},
  "message": "3-4 sentence supportive but honest weekly reflection."
}`

  const raw = await callClaude(prompt, COO_SYSTEM, 800)
  return parseJSON(raw)
}
...
Respond ONLY with JSON:
{
  ...
  const raw = await callClaude(prompt, COO_SYSTEM, weeklyCheckin ? 800 : 300)
  return parseJSON(raw)
}
...
` : 'No Oura data available — use user-reported energy level'
  ...
  const peakHours = userCtx.peak_hours || '8-10am, 2-4pm'
...
4-WEEK ROADMAP: ${roadmap || userCtx.roadmap || 'Land a Data Science (ML) role'}
...
- Peak focus windows (${peakHours}): Do/Interview/Learning tasks ONLY
