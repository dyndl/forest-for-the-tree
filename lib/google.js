...
// ── CALENDAR ────────────────────────────────────────────────────────────────
async function fetchCalendarEvents(accessToken, refreshToken, accountLabel = 'personal') {
...
export async function getTodayEvents(accessToken, refreshToken, linkedAccounts = []) {
  const [primary, ...secondaries] = await Promise.all([
    fetchCalendarEvents(accessToken, refreshToken, 'personal'),
    ...linkedAccounts
      .filter(a => a.use_calendar && a.access_token)
      .map(a => fetchCalendarEvents(a.access_token, a.refresh_token, a.label || a.email)),
  ])
  // Merge and sort by start time, deduplicate by title+start
  const all = [...primary, ...secondaries.flat()]
  const seen = new Set()
  return all
...
export async function getImportantEmails(accessToken, refreshToken) {
  try {
    ...
    const queryGroups = [
      // ── HUMAN MESSAGES — highest vigilance ───────────────────────────────
      { category: 'voicemail', q: 'from:voice-noreply@google.com newer_than:7d is:unread', max: 5 },
      { category: 'human_sent', q: 'is:inbox is:unread label:inbox newer_than:3d -has:unsubscribe -from:(noreply OR no-reply OR notifications@ OR alerts@ OR automated@ OR donotreply OR mailer-daemon OR postmaster) -from:(linkedin.com OR greenhouse.io OR lever.co OR workday.com)', max: 8 },
      // ── JOB / CAREER ─────────────────────────────────────────────────────
      { category: 'interview', q: 'subject:(interview OR "phone screen" OR "technical screen" OR offer OR rejection OR "next steps" OR "moving forward" OR "availability" OR "schedule a call" OR "excited to" OR "love to chat") newer_than:5d', max: 6 },
      { category: 'job_application', q: 'from:(greenhouse.io OR lever.co OR ashby.com OR workday.com OR bamboohr.com OR smartrecruiters.com OR icims.com OR taleo.net) newer_than:7d', max: 5 },
      { category: 'linkedin', q: 'from:(linkedin.com OR jobalerts-noreply@linkedin.com OR jobs-noreply@linkedin.com) newer_than:3d is:unread', max: 6 },
      // ── ACTION ITEMS ─────────────────────────────────────────────────────
      { category: 'action_required', q: 'subject:("action required" OR "response needed" OR "please reply" OR "following up" OR "your application" OR "we have reviewed") newer_than:5d is:unread', max: 5 },
      { category: 'important_unread', q: 'is:important is:unread newer_than:2d label:inbox -has:unsubscribe -from:(noreply OR no-reply OR notifications@)', max: 6 },
      { category: 'newsletter', q: 'has:unsubscribe older_than:7d is:unread label:inbox -label:important', max: 10 },
    ]

    const results = []
    for (const group of queryGroups) {
      try {
        const list = await gmail.users.messages.list({ userId: 'me', q: group.q, maxResults: group.max })
        const messages = (list.data.messages || []).slice(0, group.max)
        for (const msg of messages) {
          ...
          results.push({
            ...
            from: get('From'),
            date: get('Date'),
            snippet: (full.data.snippet || '').slice(0, 200),
            category: group.category,
          })
        }
      } catch { /* continue to next group */ }
    }

    const seen = new Set()
    return results.filter(e => { if (seen.has(e.id)) return false; seen.add(e.id); return true })
  } catch (err) {
    console.error('getImportantEmails error:', err.message)
    return []
  }
}

// Checks if Google Voice voicemails are flowing into Gmail
// Returns { active: bool, count: int, latest: string|null }
export async function testGoogleVoice(accessToken, refreshToken) {
  try {
    const auth = getOAuthClient(accessToken, refreshToken)
    const gmail = google.gmail({ version: 'v1', auth })
    const res = await gmail.users.messages.list({
      userId: 'me',
      q: 'from:voice-noreply@google.com newer_than:30d',
      maxResults: 3,
    })
    const messages = res.data.messages || []
    if (!messages.length) return { active: false, count: 0, latest: null }
    const full = await gmail.users.messages.get({ userId: 'me', id: messages[0].id, format: 'metadata', metadataHeaders: ['Subject', 'Date'] })
    const headers = full.data.payload?.headers || []
    return {
      active: true,
      count: res.data.resultSizeEstimate || messages.length,
      latest: headers.find(h => h.name === 'Subject')?.value || null,
    }
  } catch {
    return { active: false, count: 0, latest: null }
  }
}

export async function archiveEmails(accessToken, refreshToken, messageIds) {
  if (!messageIds?.length) return { archived: 0 }
  try {
    const auth = getOAuthClient(accessToken, refreshToken)
    const gmail = google.gmail({ version: 'v1', auth })
    ...
    for (const chunk of chunks) {
      await gmail.users.messages.batchModify({ userId: 'me', requestBody: { ids: chunk, removeLabelIds: ['INBOX'] } })
      archived += chunk.length
    }
    return { archived }
  } catch (err) {
    console.error('archiveEmails error:', err.message)
...
/** Fetch job-related emails for the 30-day backlog (used by jobs cron, not morning brief). */
export async function getJobBacklogEmails(accessToken, refreshToken) {
  try {
    const auth = getOAuthClient(accessToken, refreshToken)
    const gmail = google.gmail({ version: 'v1', auth })
    const queryGroups = [
      { category: 'linkedin',         q: 'from:(linkedin.com OR jobalerts-noreply@linkedin.com OR jobs-noreply@linkedin.com) newer_than:30d', max: 20 },
      { category: 'job_application',  q: 'from:(greenhouse.io OR lever.co OR ashby.com OR workday.com OR bamboohr.com OR smartrecruiters.com OR icims.com OR taleo.net) newer_than:30d', max: 15 },
      { category: 'interview',        q: 'subject:(interview OR "phone screen" OR "technical screen" OR offer OR rejection OR "next steps") newer_than:14d', max: 10 },
    ]
    const results = []
    for (const group of queryGroups) {
      try {
        ...
        for (const msg of messages) {
          ...
          const headers = full.data.payload?.headers || []
          const get = name => headers.find(h => h.name === name)?.value || ''
          results.push({ id: msg.id, subject: get('Subject'), from: get('From'), date: get('Date'), snippet: (full.data.snippet || '').slice(0, 200), category: group.category })
        }
      } catch { /* continue */ }
    }
    const seen = new Set()
    ...
    console.error('getJobBacklogEmails error:', err.message)
    return []
  }
}

/** Write an all-day "Weekly Plan" event to Google Calendar. Returns event ID. */
export async function writeWeeklyPlanEvent(accessToken, refreshToken, digest) {
  const auth = getOAuthClient(accessToken, refreshToken)
  const calendar = google.calendar({ version: 'v3', auth })
  const today = new Date()
  const dateStr = today.toISOString().slice(0, 10)

  const wins = (digest.wins || []).map(w => `• ${w}`).join('\n')
  const focus = (digest.next_week_focus || []).map((f, i) => `${i + 1}. ${f}`).join('\n')
  const description = `${digest.message || ''}\n\nWINS:\n${wins}\n\nNEXT WEEK:\n${focus}`

  try {
    const res = await calendar.events.insert({
      calendarId: 'primary',
      requestBody: {
        summary: `🌲 Weekly Plan — ${digest.headline || 'Weekly review'}`,
        description,
        start: { date: dateStr },
        end: { date: dateStr },
        colorId: '2',
    ...
    console.error('writeWeeklyPlanEvent error:', err.message)
    return null
  }
}
