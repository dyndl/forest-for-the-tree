...
export async function writeCOOScheduleToCalendar(accessToken, refreshToken, schedule) {
  const auth = getOAuthClient(accessToken, refreshToken)
  const calendar = google.calendar({ version: 'v3', auth })
  ...
  // Midday check-in
  const midday = new Date(today); midday.setHours(12, 0, 0, 0)
  ...
  // Afternoon pulse
  const afternoon = new Date(today); afternoon.setHours(16, 0, 0, 0)
  ...
  // Evening retro
  const evening = new Date(today); evening.setHours(19, 0, 0, 0)
...
/** Inbox stats: unread count, subscription-tagged count, total inbox size */
export async function scanGmailInbox(accessToken, refreshToken) {
  const auth = getOAuthClient(accessToken, refreshToken)
  const gmail = google.gmail({ version: 'v1', auth })
  try {
    const [unreadRes, subRes, totalRes] = await Promise.all([
      gmail.users.messages.list({ userId: 'me', labelIds: ['INBOX', 'UNREAD'], maxResults: 1 }),
      gmail.users.messages.list({ userId: 'me', q: 'has:unsubscribe', maxResults: 1 }),
...
/** Count events in the next 14 days */
